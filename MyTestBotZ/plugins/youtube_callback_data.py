    ...
    if cb_data.startswith("ytdata||"):
        ...
        if media_type == 'audio':
            buttons = InlineKeyboardMarkup([[InlineKeyboardButton(
                "ᴀᴜᴅɪᴏ", callback_data=f"{media_type}||{format_id}||{yturl}"), InlineKeyboardButton("Document",
                                                                                                    callback_data=f"docaudio||{format_id}||{yturl}")]])
        else:
            buttons = InlineKeyboardMarkup([[InlineKeyboardButton(
                "ᴠɪᴅᴇᴏ", callback_data=f"{media_type}||{format_id}||{yturl}"), InlineKeyboardButton("Document",
...
async def catch_youtube_dldata(c, q):
    ...
    await q.edit_message_reply_markup(
        InlineKeyboardMarkup([[InlineKeyboardButton("Dᴏᴡɴʟᴏᴀᴅɪɴɢ...⚡", callback_data="down")]]))
...
async def send_file(c, q, med, filename):
    ...
    try:
        await q.edit_message_reply_markup(
            InlineKeyboardMarkup([[InlineKeyboardButton("📤 Uᴘʟᴏᴀᴅɪɴɢ ᴛᴏ ᴛɢ...", callback_data="down")]]))
